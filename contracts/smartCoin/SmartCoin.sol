...
/**
 * @dev Used when generated hash already exists
 */
error HashCollision();
/**
    ...
    ISmartCoin
{
    ...
    function mint(
        address _to,
        uint256 _amount
    ) external override onlyRegistrar onlyNotFrozen(_to) returns (bool) {
    ...
    function _initiateTransferRequest(
        ...
        if(_transfers[transferHash].status != TransferStatus.Undefined) revert HashCollision();
    ...
    function _safeTransfer(
        ...
        address _to,
        uint256 _value
    ) internal onlyNotFrozen(_from) onlyNotFrozen(_to) {
